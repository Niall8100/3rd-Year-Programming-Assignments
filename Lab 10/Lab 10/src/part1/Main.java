...
public class Main 
{
	...
	public static void main(String[] args) throws InterruptedException
	{
		...
		Queue queue = new Queue(5);	//initialize queue
		ReentrantLock lock = new ReentrantLock(); //lock for use so a consume can't happen before a produce
		Condition condition = lock.newCondition();
		ExecutorService executor = Executors.newFixedThreadPool(randThreadCount);//creates thread pool that reuses a fixed number of threads
